...
export default async function handler(req: Request) {
  ...
  if (civId) {
    ...
    const { data: civ } = await supabase
      .from('civilizations')
      .select('name, flag, shortDescription, buildOrders')
      .eq('id', civId)
      .single();

    if (civ) {
      title = `Guida Civiltà ${civ.name}`;
      description = civ.shortDescription || `Strategie e build orders per ${civ.name}.`;
      image = civ.flag;
      subtitle = 'Guida Civiltà';

      if (boId && civ.buildOrders) {
        const bo = (civ.buildOrders as any[]).find(b => b.id === boId);
        if (bo) {
