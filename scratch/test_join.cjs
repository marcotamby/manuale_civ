...
async function check() {
    const { data, error } = await supabase.from('user_bets').select('*, betting_markets(*)').limit(1);
    if (error) {
        console.log('ERRORE NELLA JOIN:', error.message);
    } else {
        console.log('JOIN RIUSCITA:', data);
    }
}
check();
