              ...
              >
                <div className="h-48 relative overflow-hidden cursor-pointer" onClick={() => t.config.directLink ? window.open(t.config.directLink, '_blank') : navigate(`/tornei/${t.slug}`)}>
                    ...
                    <div className="absolute top-4 right-4 flex flex-col items-end gap-2 z-20">
                       ...
                       <div 
                        className={clsx(
                          "px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider backdrop-blur-md border border-slate-300/60 shadow-lg flex items-center gap-2 transition-all",
                          status === 'In corso' ? "bg-green-500/40 text-green-400" : 
                          status === 'Programmato' ? "bg-blue-500/40 text-blue-400" :
                          "bg-red-500/40 text-red-400"
                        )}
                        style={{ textShadow: '0 0 2px rgba(255,255,255,0.3), 0 1px 4px rgba(0,0,0,0.8)' }}
                      >
