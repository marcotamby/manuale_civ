...
import { Save, Plus, X, ChevronDown, Loader2, RefreshCcw, Sparkles, Users, HelpCircle, Trash2, Edit2, Check, Trophy } from 'lucide-react';
...
function CoachCivSelect({ value, onChange }: { value: string[]; onChange: (nextValue: string[]) => void }) {
    const [isOpen, setIsOpen] = useState(false);
    const dropdownRef = useRef<HTMLDivElement>(null);
    const selectedCivs = civilizationsData.filter(civ => value.includes(civ.id));
    ...
    const toggleCiv = (civId: string) => {
        if (value.includes(civId)) {
            onChange(value.filter(id => id !== civId));
            return;
        }

        onChange([...value, civId]);
    };

    return (
        <div className="relative w-full max-w-[420px]" ref={dropdownRef}>
            <div
                onClick={() => setIsOpen(!isOpen)}
                className="flex items-center gap-2 bg-[#0d111a] border border-white/10 rounded-xl cursor-pointer hover:border-blue-500/50 transition-all px-5 py-4"
            >
                <div className="flex-shrink-0 w-10 h-7 rounded bg-black/40 overflow-hidden border border-white/10 shadow-inner flex items-center justify-center">
                    {selectedCivs[0] ? (
                        <img src={selectedCivs[0].flag} className="w-full h-full object-cover" alt={selectedCivs[0].name} />
                    ) : (
                        <Trophy className="w-full h-full p-1 text-gray-800" />
                    )}
                </div>
                <span className="flex-1 font-black uppercase tracking-wider text-white truncate text-sm">
                    {selectedCivs.length === 0 ? "SELEZIONA CIVILTA'" : selectedCivs.length === 1 ? selectedCivs[0].name : `${selectedCivs.length} CIVILTA'`}
                </span>
                <ChevronDown className={`text-gray-600 transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`} size={16} />
            </div>

            {isOpen && (
                <div className="absolute left-0 z-50 mt-2 w-[420px] max-w-[calc(100vw-2rem)] bg-[#0d111a] border border-white/20 rounded-2xl shadow-2xl overflow-hidden max-h-72 overflow-y-auto elegant-scrollbar animate-in fade-in zoom-in-95 duration-150">
                    <button
                        type="button"
                        onClick={() => {
                            onChange([]);
                        ...
                        className="flex items-center gap-4 px-4 py-3 hover:bg-blue-600/10 transition-all cursor-pointer group border-b border-white/5 w-full text-left"
                    >
                    ...
                    {civilizationsData.map(civ => {
                        const isSelected = value.includes(civ.id);
                        ...
                        return (
                            <button
                                ...
                                className={`flex items-center gap-4 px-4 py-3 transition-all cursor-pointer group border-b border-white/5 last:border-0 w-full text-left ${isSelected ? 'bg-blue-600/10' : 'hover:bg-blue-600/10'}`}
                            >
                                <div className="w-10 h-7 rounded overflow-hidden border border-white/10 group-hover:border-blue-500/30 transition-all shadow-md flex-shrink-0">
                                    <img src={civ.flag} className="w-full h-full object-cover" alt={civ.name} />
                                </div>
                                <span className="flex-1 text-xs font-black text-gray-400 group-hover:text-white uppercase tracking-widest truncate">{civ.name}</span>
                                <span className={`text-[10px] font-black uppercase tracking-widest ${isSelected ? 'text-blue-400' : 'text-gray-600'}`}>
                                    {isSelected ? 'Selezionata' : 'Aggiungi'}
                                </span>
...
export function DraftMatchingDashboard({ onError }: DraftMatchingDashboardProps) {
    ...
    const addCoach = () => {
        ...
        const newCoach: Coach = { id: `c-${Date.now()}`, name: newCoachName.trim(), civIds: [] };
        setState(prev => ({ ...prev, coaches: [...prev.coaches, newCoach] }));
        setNewCoachName('');
    };
    ...
    const addStudent = () => {
        ...
        const newStudent: Student = { id: `s-${Date.now()}`, name: newStudentName.trim() };
    ...
    const handleCasterNameChange = (idx: number, name: string) => {
        setState(prev => {
            ...
            return { ...prev, casters: newCasters };
        });
    };
    ...
    const handleToggleLiveSync = () => {
        setState(prev => ({ ...prev, liveSync: !prev.liveSync }));
    };
        ...
        <div className="flex flex-col bg-[#05080f] font-inter text-white min-h-screen">
                ...
                <div className="flex items-center gap-4">
                    ...
                    {!showResetConfirm ? (
                        ...
                        <div className="flex items-center gap-2 bg-red-600 p-1 rounded-xl shadow-lg animate-in zoom-in-95 duration-200">
                            ...
                            <button onClick={handleReset} className="bg-white text-red-600 px-4 py-2 rounded-lg font-black text-[10px] uppercase hover:bg-gray-100 transition-all">
                                SI, RESET
                            </button>
                            <button
                ...
                <div className="flex items-center gap-4 bg-black/60 px-6 py-3 rounded-2xl border border-white/5 shadow-inner">
                    <div className="flex items-center gap-2">
                        <span className="relative flex h-2 w-2">
                            {state.liveSync && <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>}
            ...
            <div className="p-8 pb-32 space-y-8">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <div className="bg-[#0a0f1a] border border-white/10 rounded-3xl p-6 shadow-2xl flex flex-col w-full h-[650px]">
                        ...
                        <div className="flex-1 overflow-y-auto elegant-scrollbar space-y-2 pr-1">
                            {state.coaches.length === 0 ? (
                                ...
                                state.coaches.map(coach => {
                                    ...
                                    const pairingColor = pairingIndex !== -1 ? PAIRING_COLORS[pairingIndex % PAIRING_COLORS.length] : null;
                                            ...
                                            {editingCoachId === coach.id ? (
                                                ...
                                                <div className="flex items-center justify-between gap-3 w-full">
                                                    <div className="min-w-0">
                                                        <span className="font-bold text-xs uppercase tracking-wide truncate max-w-[200px]">{coach.name}</span>
                                                        <div className="mt-2">
                                                            <CoachCivSelect value={coach.civIds} onChange={(civIds) => handleCoachCivChange(coach.id, civIds)} />
                                                        </div>
                                                    </div>
                                            ...
                                            {editingStudentId === student.id ? (
                                                ...
                                                <>
                                                    <div className="flex flex-col min-w-0">
                                                        <span className="font-bold text-xs uppercase tracking-wide truncate max-w-[180px]">{student.name}</span>
                        ...
                        <div className="flex-1 overflow-y-auto elegant-scrollbar space-y-4 pr-1">
                            {state.coaches.length === 0 ? (
                                <div className="h-full flex flex-col items-center justify-center text-gray-600 gap-2 text-center p-4">
                                    <HelpCircle size={24} className="opacity-35" />
                                    <span className="text-[10px] font-black uppercase tracking-wider">Inserisci i Coach e gli Allievi nelle colonne a sinistra per creare gli abbinamenti</span>
                                </div>
                            ) : (
                                state.coaches.map(coach => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className="flex items-center justify-between">
                                                <span className="text-[10px] font-black text-cyan-400 uppercase tracking-wider">Coach: {coach.name}</span>
                                                {pairing && (
                                                    <button
                                            ...
                                            <div className="relative">
                                                ...
                                                >
                                                    ...
                                                    {state.students.map(student => {
                                                        ...
                                                        return (
                                                            <option key={student.id} value={student.id} className="bg-[#0d111a]">
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div className="space-y-1 relative" ref={fontDropdownRef}>
                                    ...
                                    >
                                        <span style={{ fontFamily: state.titleFont }} className="tracking-wide uppercase">{state.titleFont}</span>
                                                ...
                                                {[
                                                    ...
                                                    return (
                                                        <button
                                                            ...
                                                            className={`w-full flex items-center justify-between px-3 py-2 rounded-lg transition-all text-left ${isSelected ? 'bg-purple-600/20 border border-purple-500/30 text-white' : 'hover:bg-white/5 border border-transparent text-gray-300 hover:text-white'}`}
                                                        >
                                                            <div className="flex flex-col">
                                                                <span style={{ fontFamily: item.value }} className="text-xs font-bold uppercase tracking-wider">{item.label}</span>
                                                                <span className="text-[8px] text-gray-500 font-medium">{item.desc}</span>
                                                            </div>
