...
export function Topbar({ onOpenAdminDashboard, onOpenAdminOverlay, isHome }: TopbarProps) {
  ...
  const isSpecialPage = isHome || location.pathname.includes('/tornei') || location.pathname === '/faq' || location.pathname.startsWith('/civ/') || location.pathname.startsWith('/compare');

  return (
    <div className="w-full flex flex-col shrink-0 z-[100] relative select-none">
      
      {/* 1. Desktop Utility Topbar (PC Only) */}
      <div className="hidden lg:flex w-full bg-black/40 backdrop-blur-md border-b border-white/5 h-12 items-center px-14 justify-between relative z-[110]">
         ...
         <div className="flex items-center gap-4">
            <a
              ...
              className="flex items-center gap-2 text-indigo-400 hover:text-indigo-300 transition-colors text-[11px] font-bold uppercase tracking-wider group"
            >
            ...
            <Link to="/tornei" className="flex items-center gap-2 text-emerald-400 hover:text-emerald-300 transition-colors text-[11px] font-bold uppercase tracking-wider">
              <Trophy size={14} /> Tornei
            </Link>
            <div className="w-px h-3 bg-white/10"></div>
            <Link to="/faq" className="flex items-center gap-2 text-sky-400 hover:text-sky-300 transition-colors text-[11px] font-bold uppercase tracking-wider">
              <HelpCircle size={14} /> FAQ
            </Link>
            <div className="w-px h-3 bg-white/10"></div>
            <a href="https://ko-fi.com/marcotamby" target="_blank" className="flex items-center gap-2 text-rose-400 hover:text-rose-300 transition-colors text-[11px] font-bold uppercase tracking-wider">
              <Coffee size={14} /> Sostieni
            </a>
            {(isAdmin || isStreamer) && user?.email !== 'alessio.bella97@gmail.com' && (
              <>
                <div className="w-px h-3 bg-white/10"></div>
                <button onClick={onOpenAdminOverlay} className="flex items-center gap-2 text-fuchsia-400 hover:text-fuchsia-300 transition-colors text-[11px] font-bold uppercase tracking-wider">
            ...
            {isAuthenticated ? (
              <div className="flex items-center gap-4">
                ...
                >
                  ...
                  {notificationCount > 0 && (
                    <span className="absolute -top-1 -left-1 z-10 flex h-3 w-3 items-center justify-center rounded-full bg-red-600 text-[7px] font-black text-white shadow-[0_0_5px_rgba(220,38,38,0.5)] ring-1 ring-black">
                      {notificationCount}
                    </span>
                  )}
                  <span className="text-[10px] font-bold text-white uppercase tracking-widest">{user?.nickname || 'Profilo'}</span>
                </button>

                {isAdmin && (
                  <button onClick={onOpenAdminDashboard} className="relative text-[10px] bg-yellow-500/10 text-yellow-500 border border-yellow-500/30 px-3 py-1 rounded-md font-black uppercase tracking-widest hover:bg-yellow-500 hover:text-black transition-all">
                    Pannello
                ...
                <button onClick={() => setShowLogoutConfirm(true)} className="text-[10px] text-gray-500 hover:text-white transition-colors font-bold uppercase tracking-widest">
                  Esci
                </button>
              </div>
            ) : (
              <button onClick={() => openLoginModal()} className="text-[10px] bg-yellow-600/10 text-yellow-500 border border-yellow-500/30 px-4 py-1.5 rounded-lg font-bold uppercase tracking-widest hover:bg-yellow-500 hover:text-black transition-all">
                Accedi
              </button>
            )}
         </div>
